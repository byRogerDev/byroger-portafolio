...
export function ElementCV(element) {

    const isCurrent = (moment(element.end).format('MM/Y') === moment().format('MM/Y'))?true:false;

    const classAux = (element.index && element.index%2 !== 0)?'relative  md:pt-20':'relative'

    return (<div className={classAux}>
    <dt>
      ...
      <p className="ml-16 text-lg font-medium leading-6 text-gray-900 text-left">{element.name}</p>
    </dt>
    {element.start == null && <dd className="mt-2 ml-16 text-base text-gray-500 text-left"><strong>{element.year}</strong></dd> }
    ...
    {element.description && Array.isArray(element.description) && element.description.map( ( ele, index) => (
    ...
    {element?.techs?.map(({ name, img }, index) => (
